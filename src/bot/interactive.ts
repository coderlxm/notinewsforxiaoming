    ...
    try {
      ...
      await ctx.telegram.editMessageText(
        ...
        [
          ...
          `命中格式：${result.selectedFormat.formatId} · ${result.selectedFormat.description}`,
          `视频：${result.selectedFormat.resolution} · ${result.selectedFormat.fps ?? '未知'} fps · ${result.selectedFormat.videoCodec ?? '未知编码'}`,
          `音频：${result.selectedFormat.audioCodec ?? '未知编码'} · 封装：${result.selectedFormat.container} · ${result.selectedFormat.dynamicRange ?? '未知动态范围'}`,
    ...
    try {
      ...
      if (summary.status === 'interest_pending') {
        await ctx.telegram.editMessageText(
          ...
          startMessageResult.message_id,
          undefined,
          `发现 ${summary.pendingProjects} 个尚未确认兴趣的项目。请先在项目确认卡中选择，选择前不会启动赛事推送。`,
          { parse_mode: 'HTML' },
        );
        return;
      }
      if (summary.status === 'interest_dismissed') {
        await ctx.telegram.editMessageText(
          String(ctx.chat!.id),
          startMessageResult.message_id,
          undefined,
          '本次发现的赛事均已选择不关注本届，未启动赛事推送。',
          { parse_mode: 'HTML' },
        );
        return;
      }
