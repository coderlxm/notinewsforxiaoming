...
- 状态：图片 Blur-up 已实施并完成首屏接管修正；视频首帧 Blur-up 另见独立方案
...
实施结论（2026-07-23）：真实条目进入 DOM 后，`@egjs/grid` 默认会等待卡片内部图片和视频达到媒体就绪状态，再触发 `renderComplete`。实际首屏曾因此在接口返回后继续停留骨架约 4.52 秒。最终在每个 `.waterfall__item` 上使用库原生 `data-grid-skip="true"`，让 Masonry 直接测量已经由宽高比稳定占位的卡片盒子，媒体加载不再阻塞瀑布流接管；低清图片立即请求，原图继续懒加载。该规则是当前实现的一部分，后续媒体功能不得移除或改回等待内部媒体。
