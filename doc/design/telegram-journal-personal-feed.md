...
Telegram 相册中的每个 `message_id` 独立保存，并通过同一个 `media_group_id` 关联。Web API 将同组记录聚合为一条信息流卡片；可见性切换和置顶操作对整组生效。
...
初版不在 Telegram 和 Web 暴露删除入口。个人资产误删的损失大于暂时保留一条错误记录；正文可编辑，错误公开可以立即转私有。

如果后续确实需要删除，再单独设计数据库记录、附件目录和媒体组的一致删除行为，不先加入软删除状态机。
...
在 `rndc02` 增加公开 API、受控媒体路由、Vue 单列信息流、`/me` 登录、搜索筛选、正文编辑、置顶、往年今日、RSS 和 JSON Feed。
