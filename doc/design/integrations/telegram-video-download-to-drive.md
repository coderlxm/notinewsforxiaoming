...
批量下载继续复用 `/download`，不增加 `/downloadbatch` 等独立命令。单条下载保持现有写法；批量下载时，在同一条消息里每行填写一个 URL：
...
首版批量交互建议最多接收 10 个 URL。这里只把显式填写的 URL 视为批次条目，播放列表和频道链接仍不展开，避免一次命令隐式产生大量任务。

批次按输入顺序串行处理，并沿用现有的单任务锁。每个 URL 仍按当前规则选择执行端：哔哩哔哩链接交给 Mac，其他支持的网站交给 `bwgdc01`。不并发下载，也不额外引入队列系统。

Telegram 中始终编辑同一张批次状态卡，不为每个视频新增消息。处理中展示当前序号、当前视频和阶段，例如：
...
总耗时：12 分 30 秒
全程平均速度：2.5 MB/s
目录：NotiNewsDownloads/2026-07
```

任意一条失败时，批次立即停止并直接展示原始错误，不自动重试、不跳过失败条目，也不继续处理剩余链接。已经上传成功的文件保留：
...
重复 URL 不静默去重，按用户输入依次执行；如果最终文件名相同，则沿用当前 Google Drive 同名文件覆盖行为。首版不增加暂停、取消、重排、历史批次和失败项续跑等状态管理。
